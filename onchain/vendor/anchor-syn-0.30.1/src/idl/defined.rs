...
pub fn gen_idl_type(
    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            {
                // Disabled in BlockMine's local vendored build because newer proc-macro2
                // releases removed `Span::source_file()`, which Anchor 0.30.1 still calls
                // behind `procmacro2_semver_exempt`. This fallback only affects optional
                // alias/external-type resolution for IDL generation and is not needed by
                // the current program types.
            }
